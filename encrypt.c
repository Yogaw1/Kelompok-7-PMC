...
int main() {
    ...
    printf("Enter the secret key (32 bytes): ");
    if (fgets((char *) key, CRYPTO_KEYBYTES + 1, stdin) == NULL) {
        printf("Error: Failed to read key\n");
        return -1;
    }
    key[CRYPTO_KEYBYTES] = '\0';

    // Get the nonce from the user
    printf("Enter the nonce (16 bytes): ");
    if (fgets((char *) nonce, CRYPTO_NPUBBYTES + 1, stdin) == NULL) {
        printf("Error: Failed to read nonce\n");
        return -1;
    }
    nonce[CRYPTO_NPUBBYTES] = '\0';

    // Get the plaintext from the user
    printf("Enter the plaintext: ");
    if (fgets((char *) plaintext, 100 + 1, stdin) == NULL) {
        printf("Error: Failed to read plaintext\n");
        return -1;
    }
    plaintext[strcspn((char *) plaintext, "\n")] = '\0';

    // Get the associated data from the user
    printf("Enter the associated data: ");
    if (fgets((char *) ad, 100 + 1, stdin) == NULL) {
        printf("Error: Failed to read associated data\n");
        return -1;
    }
    ad[strcspn((char *) ad, "\n")] = '\0';
